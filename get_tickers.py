# app.py (Final Version - with Stock Info Dashboard)
...
with col1:
    if tickers_df is not None:
        try:
            ...
            default_index = 0
        selected_display = st.selectbox('Search for a stock', options=tickers_df['DISPLAY'], index=default_index, label_visibility="collapsed")
    ...
    with st.spinner('Running forecast... Please wait.'):
        try:
            ...
            if X_test is None:
                ...
                # --- NEW: STOCK INFORMATION SECTION ---
                st.divider()
                st.subheader(f"Key Information for {stock}")
                stock_info = get_stock_info(stock)
                if stock_info:
                    info_col1, info_col2, info_col3 = st.columns(3)
                    info_col1.metric("Market Cap", f"₹{stock_info.get('marketCap', 0):,}")
                    info_col2.metric("52 Week High", f"₹{stock_info.get('fiftyTwoWeekHigh', 0):.2f}")
                    info_col3.metric("52 Week Low", f"₹{stock_info.get('fiftyTwoWeekLow', 0):.2f}")
                ...
                metric_col1.metric("📊 Model Accuracy", f"{accuracy:.2f}%")
                metric_col2.metric("📉 MAPE", f"{mape:.2f}%")
                metric_col3.metric("📈 R² Score", f"{r_squared:.4f}")
                ...
                # --- UPDATED: More descriptive graph title ---
